...
class Transformer(nn.Module):
    ...
    def __init__(self, d_model: int = 512, nhead: int = 2, num_encoder_layers: int = 2,
    ...
    def forward(self, src: Tensor, tgt: Optional[Tensor] = None, src_mask: Optional[Tensor] = None,
        ...
        #memory = src.view(-1, 64, 32)
        # 1 512
        memory_encoder = self.fc_encoder(memory.view(memory.shape[0], -1))
        # memory_encoder = self.fc_encoder1(memory_encoder)
        # 1 64 32
...
def transnet(reduction=64):
    ...
    r""" Create a proposed TransNet.
        ...
        """
    model = Transformer(d_model=32,num_encoder_layers=2,num_decoder_layers=3,nhead=2,reduction =reduction,dropout= 0.)
    return model
